    ...
    public class StaffController : Controller
    {
        ...
        public IActionResult EditGame1(Games game, IFormFile imageFile)
        {
            if (game == null || string.IsNullOrEmpty(game.Name) || string.IsNullOrEmpty(game.Game_Description) || game.SubCatID == 0 || game.price == 0)
            {
                ...
                return View(game); // Return to the edit form with errors
            }

            string filePath = game.image; // Keep existing image path if no new file is uploaded
            ...
            if (imageFile != null && imageFile.Length > 0)
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(game.image) && game.image != "/uploads/" + imageFile.FileName) // Basic check, improve if needed
                    {
            ...
            using (SqlConnection connection = new SqlConnection(this.configuration.GetSection("ConnectionStrings")["DefaultConnection"]))
            {
                ...
                try
                {
                    ...
                    transaction.Commit();
                    string script = "<script>alert('Game details updated successfully!');window.location='/Admin/Games';</script>";
                    return Content(script, "text/html");
                }
                catch (Exception ex)
                {
                    transaction.Rollback();
                    TempData["Error"] = $"Failed to update game details: {ex.Message}";
                    return View("Games");
                }
            }
        }
